	...
	return func(c *gin.Context) {
		...
		for _, allowed := range configuration.Config.SuperAdminAllowedIPs {
			...
			if strings.Contains(allowed, "/") {
				...
				if err != nil {
					logs.Log("[WARN][AUTHZ] Invalid CIDR notation in allowed IPs: " + allowed)
					continue
				}
		...
		if !isIPAllowed {
			logs.Log("[AUTH][ERROR] super admin access denied: IP " + clientIP + " not in allowed list")
		...
		if expectedToken == "" || providedToken == "" {
			logs.Log("[AUTH][ERROR] super admin authentication failed: missing or empty token")
		...
		if subtle.ConstantTimeCompare([]byte(providedToken), []byte(expectedToken)) != 1 {
			logs.Log("[AUTH][ERROR] super admin authentication failed: invalid token")
			c.AbortWithStatusJSON(http.StatusUnauthorized, structs.Map(models.StatusUnauthorized{
				Code:    http.StatusUnauthorized,
				Message: "super admin authentication required",
				Data:    nil,
		...
		logs.Log("[AUTH] super admin authentication success")
		c.Next()
	}
}
